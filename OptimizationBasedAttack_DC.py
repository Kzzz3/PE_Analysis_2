...
#     strStartIndex =os.listdir(strQFRoot)[0]
#     strSpanRoot = os.path.join(strQFRoot, strStartIndex)
#
#     arrSpans = os.listdir(strSpanRoot)
#     for Span in tqdm(arrSpans):
#         nQF = int(QF[QF.find("=") + 1:])
#         nSpan = int(Span[Span.find("=") + 1:])
#         nStartIndex = int(strStartIndex[strStartIndex.find("=")+1:])
#
...
#
#         #针对CipherImage 进行优化攻击
#         arrDCT,arrMask,arrDcDct,arrDcMask = PreProcess(strCipherImagePath,nStartIndex,nSpan)
#         arrRecoveredImage = OptimizationBasedAttack(arrDCT,arrMask,arrDcDct,arrDcMask)
#         cv.imwrite(strOutputImagePath, arrRecoveredImage)
#
#
#         # 计算图像psnr 和 ssim
...
#         ssim = structural_similarity(imgDst, imgSrc)
#
#         dictPSNR[int(QF[QF.find("=")+1:])][int(Span[Span.find("=") + 1:])] = psnr
#         dictSSIM[int(QF[QF.find("=")+1:])][int(Span[Span.find("=") + 1:])] = ssim
#
# #draw the result
# arrQFs = [ QF for QF in list(dictPSNR.keys())]
# arrSpans = [ST for ST in list(dictPSNR.values())[0]]
# arrSpans.sort()
#
# for QF in arrQFs:
#     print("QF="+str(QF))
#     print("PSNR:")
#     for Span in arrSpans:
#         print(Span, round(dictPSNR[QF][Span], 3), r"\\")
#     print("SSIM:")
#     for Span in arrSpans:
#         print(Span, round(dictSSIM[QF][Span], 3), r"\\")
